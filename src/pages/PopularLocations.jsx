...
export default function PopularLocations() {
  ...
  return (
    <div id="locations-page" className="pb-16 bg-[#FAFDFD] font-sans text-slate-700">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-slate-100 pb-6">
          <div className="max-w-xl">
            <span className="text-xs font-black uppercase tracking-widest text-secondary bg-secondary/10 px-3 py-1.5 rounded-full inline-block mb-3">
              Doorstep Pickup Points
            </span>
            <h2 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight mb-2">
              Select Pickup Location
            </h2>
            <p className="text-xs md:text-sm text-slate-500 font-medium leading-relaxed">
              Click on any location link below to instantly schedule a free doorstep medicine pickup, check local office availability, and get direct assistance via WhatsApp.
            </p>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <AnimatePresence mode="wait">
          {isLoading ? (
            ...
            >
              <MapPin size={48} className="mx-auto text-slate-300 mb-3 animate-pulse" />
              <h3 className="text-base font-black text-slate-800 tracking-tight">No Locations Found</h3>
              <p className="text-xs text-slate-400 mt-1 font-medium">Try searching for another city or hub name.</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      {showScrollTop && (
        <button
          ...
          className="fixed bottom-6 right-6 z-40 bg-secondary hover:bg-[#02948c] text-white p-3.5 rounded-full shadow-xl hover:scale-110 active:scale-95 transition-all duration-300 focus:outline-none"
          title="Scroll to Top"
        >
          <ArrowUp size={20} />
        </button>
