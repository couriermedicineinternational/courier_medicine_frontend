...
import { Save, Info, CheckCircle, FileText, Globe } from "lucide-react";
...
export default function AdminRefundPolicy() {
  ...
  const fetchPolicy = async () => {
    try {
      ...
      if (res.data && res.data.success && res.data.data) {
        ...
        setTitle(d.title || "Refund Policy");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.put("/refund-policy", { title, bgImage, content });
      ...
      setSaveError(err.response?.data?.message || "Failed to save Refund Policy.");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  return (
    <div id="admin-refund-policy-page" className="max-w-4xl mx-auto space-y-6 font-sans">
      ...
      {saveSuccess && (
        <div className="flex items-center gap-2.5 bg-emerald-50 border border-emerald-200 text-emerald-700 p-4 rounded-2xl text-xs font-bold animate-slideDown shadow-sm">
          ...
          <span>Refund Policy updated successfully! Changes are live on the public page.</span>
        </div>
      )}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-1 pt-4 border-t border-slate-100">
            ...
            <RichTextEditor
              ...
              placeholder="Enter detailed refund policy content..."
              height={450}
            />
          </div>
