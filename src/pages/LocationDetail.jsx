...
export default function LocationDetail() {
  ...
  // Find route data from params
  const route = ALL_LOCATIONS.find(
    l => l.slug === slug
  );

  // Find country data to get basePrice and customs advice
  const country = route ? ALL_COUNTRIES.find(
    c => c.name.toLowerCase() === route.country.toLowerCase() || c.code.toLowerCase() === route.country.toLowerCase()
  ) : null;
  ...
  if (!route) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center p-6 text-center">
        ...
        <Link to="/popular-locations.htm" className="px-5 py-2.5 bg-primary text-white text-xs font-bold rounded-xl shadow">
  ...
  const originCity = route.city;
  const destCountry = route.country;

  // Pricing calculations
  const calculatePrice = (bPrice, weight, carrierMultiplier = 1) => {
    let finalPrice = bPrice * carrierMultiplier;
    if (weight > 0.5) {
  ...
  const dhlPrice = calculatePrice(basePrice, packageWeight, 1);
  const upsPrice = calculatePrice(basePrice, packageWeight, 0.95);
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full text-center text-white relative z-20 mt-12">
              ...
              >
                ...
                <nav className="text-xs md:text-sm font-semibold tracking-wide font-sans flex items-center justify-center gap-2 text-white/80 mt-6">
                  ...
                  <Link to="/popular-locations.htm" className="hover:text-secondary transition-colors text-white/95">Locations</Link>
                  <span className="text-white/40">»</span>
            ...
            >
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 items-stretch">
                ...
                <div className="bg-gradient-to-br from-white to-slate-50/20 border border-slate-200/80 rounded-3xl p-6 shadow-sm flex flex-col justify-between relative overflow-hidden group">
                  ...
                  <a
                    href={`https://wa.me/918882691919?text=Hi! I want to book DHL Medicine Courier from ${originCity} to ${destCountry} for package weight: ${packageWeight} kg. Estimated price: ₹${dhlPrice}.`}
                    target="_blank"
                ...
                <div className="bg-gradient-to-br from-white to-slate-50/20 border border-slate-200/80 rounded-3xl p-6 shadow-sm flex flex-col justify-between relative overflow-hidden group">
                  ...
                  <a
                    href={`https://wa.me/918882691919?text=Hi! I want to book UPS Medicine Courier from ${originCity} to ${destCountry} for package weight: ${packageWeight} kg. Estimated price: ₹${upsPrice}.`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-6 w-full text-center py-2.5 rounded-xl bg-secondary hover:bg-[#02948c] text-white font-bold text-xs shadow-sm hover:scale-[1.01] active:scale-[0.99] transition-all cursor-pointer block leading-none"
                  >
                    Book UPS Pickup
                  </a>
                </div>
